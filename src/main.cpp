...
int main() {
    ...
    {
        TreeNode *root = buildTree({1, 2, 5, 3, 4, nullopt, 6});
        s.flatten(root);
        vector<optional<int>> expected{1, nullopt, 2, nullopt, 3, nullopt, 4, nullopt, 5, nullopt, 6};
...

    {
        ...
        s.flatten(root);
        vector<optional<int>> expected{0};
        checkEqual("sample-3", treeToLevelOrder(root), expected);
    }
