...
int main() {
    ...
    {
        TreeNode *root = buildTree({10, 5, -3, 3, 2, nullopt, 11, 3, -2, nullopt, 1});
        checkEqual("sample-1", s.pathSum(root, 8), 3);
    }

    {
        TreeNode *root = buildTree({5, 4, 8, 11, nullopt, 13, 4, 7, 2, nullopt, nullopt, 5, 1});
        checkEqual("sample-2", s.pathSum(root, 22), 3);
    }
